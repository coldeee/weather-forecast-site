// Создание и инициализация канваса для анимации точек
const canvas = document.createElement('canvas');
canvas.id = 'backgroundCanvas'; // Установка идентификатора канваса
document.body.appendChild(canvas); // Добавление канваса в тело документа

const ctx = canvas.getContext('2d'); // Получение 2D контекста для рисования

// Установка начальной ширины и высоты канваса в соответствии с размером окна
...
// Функция для обработки изменения размера окна и обновления размеров канваса
...
function Dot() {
    this.x = Math.random() * width; // Случайная позиция по оси X
    this.y = Math.random() * height; // Случайная позиция по оси Y
...
function drawDots() {
    ctx.clearRect(0, 0, width, height); // Очистка канваса перед перерисовкой
